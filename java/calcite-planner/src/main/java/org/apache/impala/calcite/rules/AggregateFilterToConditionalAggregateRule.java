...
@Value.Enclosing public class AggregateFilterToConditionalAggregateRule
  ...
  @Override public void onMatch(RelOptRuleCall call) {
    ...
    for (AggregateCall aggCall : aggregate.getAggCallList()) {
      ...
      if (aggCall.getArgList().size() == 1) {
        thenExpr = rexBuilder.makeInputRef(filter, aggCall.getArgList().get(0));
  ...
  @Value.Immutable public interface Config extends RelRule.Config {
    Config DEFAULT = ImmutableAggregateFilterToConditionalAggregateRule.Config.builder()
        .operandSupplier(
            a -> a.operand(Aggregate.class).oneInput(f -> f.operand(Filter.class).anyInputs())).build();
