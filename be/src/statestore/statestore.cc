...
Status Statestore::SendTopicUpdate(Subscriber* subscriber, UpdateKind update_kind,
  ...
  RETURN_IF_ERROR(client.DoRpc(&StatestoreSubscriberClientWrapper::UpdateStateWithRetry,
      update_state_request, &response));
...
Status Statestore::SendHeartbeat(Subscriber* subscriber) {
  ...
  RETURN_IF_ERROR(client.DoRpc(&StatestoreSubscriberClientWrapper::HeartbeatWithRetry,
      request, &response));
  ...
  for (std::vector<std::shared_ptr<Subscriber>>::iterator it = rpc_receivers.begin();
    ...
    if (status.ok()) {
      ...
      if (status.ok()) {
        ...
        status = client.DoRpc(&StatestoreSubscriberClientWrapper::UpdateCatalogdWithRetry,
            request, &response);
        if (!status.ok()) {
          if (status.code() == TErrorCode::RPC_RECV_TIMEOUT) {
            // Add details to status to make it more useful, while preserving the stack
  ...
  for (std::vector<std::shared_ptr<Subscriber>>::iterator it = rpc_receivers.begin();
    ...
    if (status.ok()) {
      ...
      if (status.ok()) {
        ...
        status = client.DoRpc(
            &StatestoreSubscriberClientWrapper::UpdateStatestoredRoleWithRetry,
            request, &response);
