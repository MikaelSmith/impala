...
Status CTEProducerNode::Open(RuntimeState* state) {
  ...
  do {
    ...
    // It is safe to move child_batch into the exchanger even when needs_deep_copy is set
    // because Push() takes full ownership of the batch, including its tuple_data_pool_
    // and any attached buffers. The 'needs_deep_copy' flag signals that *unattached*
    // resources backing the batch may be released on the child's next GetNext() call;
    // however, for all current child node types, those unattached resources do not
    // include the batch's actual tuple data (which is in tuple_data_pool_), so the
    // exchanger's reference remains valid.
